...
const ArkAuth = (() => {
  ...
  async function makeAuthenticatedRequest(baseUrl, service, contractId, nonce, signerFn, fetchOpts = {}) {
    ...
    const url = new URL(`${baseUrl.replace(/\/$/, '')}/${service}`);
    if (url.protocol !== 'https:' || url.username || url.password) throw new Error('HTTPS provider URL required');
    const headers = new Headers(fetchOpts.headers);
    headers.set('arkauth', arkauth);
    const timeout = AbortSignal.timeout(10000);
    const signal = fetchOpts.signal ? AbortSignal.any([timeout, fetchOpts.signal]) : timeout;
    return fetch(url, { ...fetchOpts, headers, signal, redirect: 'error' });
  }

  /**
   * Create a signer function that uses the Keplr wallet.
   *
   * ADR-036 signatures do not match the on-chain PAYG preimage.
   * This adapter rejects the unsupported flow before opening a signing prompt.
   *
   ...
   */
  async function getKeplrSigner(_chainId) {
    // ADR-036 envelopes are not the raw PAYG preimage verified at settlement.
    // Do not ask the wallet to sign an authorization that cannot be settled.
    throw new Error('Keplr arbitrary signing is not compatible with PAYG settlement. Use the documented SDK signer or a dedicated delegate.');
  }
